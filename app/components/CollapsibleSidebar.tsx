...
export default function CollapsibleSidebar({ children, isAdmin = false }: { children: React.ReactNode; isAdmin?: boolean }) {
  ...
  return (
    <>
      ...
      <aside 
        id="sidebar"
        className={`h-screen bg-slate-50 border-r border-slate-200/50 z-50 transition-all duration-300 ease-in-out fixed left-0 top-0 flex flex-col ${
      ...
      >
        <div className="mb-8 px-4">
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">@username</p>
        </div>
        <nav className="flex-1 space-y-2">
          ...
          <a className="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-200 rounded-xl transition-all active:scale-95 duration-200 ease-in-out" href="#">
        ...
        <div className="mt-auto p-4 bg-indigo-700 rounded-2xl text-white">
          ...
          <button className="w-full py-2 bg-white text-indigo-700 rounded-full text-sm font-bold shadow-sm hover:bg-indigo-50 transition-colors">
            Upgrade to Pro
          </button>
        </div>
      </aside>
      ...
      <button 
        ...
        className={`fixed w-6 h-6 bg-white border border-slate-200 rounded-full shadow-md flex items-center justify-center hover:bg-slate-50 transition-all duration-300 z-50 ${
