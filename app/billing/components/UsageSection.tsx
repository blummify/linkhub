...
function fillStyle(type: string): React.CSSProperties {
  if (type === "danger") return { background: "linear-gradient(90deg, #e11d48, #be123c)" };
  if (type === "warn")   return { background: "linear-gradient(90deg, #d97706, #ea580c)" };
  return { background: "linear-gradient(90deg, #6873ff, #3b46e0)" };
}

export function UsageSection() {
  const storageItem = USAGE_ITEMS.find((i) => i.label === "Storage")!;
  const storagePct = Math.round((storageItem.current / storageItem.limit) * 100);
        ...
        >
          ...
          <button
            style={{
              ...
              border: "1px solid #d6dae9", borderRadius: 8,
